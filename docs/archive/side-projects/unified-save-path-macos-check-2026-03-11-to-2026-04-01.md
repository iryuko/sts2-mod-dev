...
> 档案状态：旁支 mod 的历史兼容性调查，不属于当前 Togawasakiko 主线。
...
- 当前工作区原先复制的参考文件确实已经落后：
  - `references/api-notes/app/release_info.json` 原先还停在 `v0.98.2`
- 已使用：
  - `shared/scripts/sync-game-files.sh`
  从当前游戏目录重新同步参考文件。
- 当前真实游戏版本仍是：
  - `v0.99.1`
  - commit `7ac1f450`
- 当前 `sts2.runtimeconfig.arm64.json` 仍是：
  - `net9.0`
  - `Microsoft.NETCore.App 9.0.7`
- 当前 `sts2.deps.arm64.json` 里与本 mod 最相关的依赖仍是：
  - `0Harmony 2.4.2.0`
  - `GodotSharp 4.5.1`
- 用同步后的当前 `sts2.dll` 重新对照后，`UnifiedSavePath` 依赖的关键接口仍然没有漂移：
  - `UserDataPathProvider.IsRunningModded`
  - `UserDataPathProvider.GetProfileDir(int)`
  - `OneTimeInitialization.ExecuteEssential()`
- `ExecuteEssential()` 当前仍然是：
  - 先 `ModManager.Initialize(...)`
  - 再 `UserDataPathProvider.IsRunningModded = ModManager.LoadedMods.Count > 0`
- 这说明：
  - 当前游戏更新没有把这个 mod 的核心挂点删掉
  - 当前的主要风险仍然是“时序 / Harmony 能否生效”，不是“API 已改坏”

### 本轮新增结论

- `UnifiedSavePath` 当前没有发现因为 `v0.99.1` 更新而产生的直接编译级 API 失配。
- 已用同步后的参考程序集重新构建：
  - `shared/scripts/build-mod.sh mods/UnifiedSavePath`
  构建通过。
- 这意味着就“对照当前游戏文件是否落后”这个问题而言：
  - 代码本身没有明显落后到需要立刻改接口
  - 但工作区参考文件此前确实落后，已经补齐
  - 游戏目录中的安装版外部 manifest 此前少了 `dependencies: []`，已经通过重装对齐到当前脚本产物

### 本轮已执行的最小修正

- 已重新构建 `UnifiedSavePath`
- 已重新安装到：
  - `SlayTheSpire2.app/Contents/MacOS/mods/UnifiedSavePath/`
- 当前安装版外部 manifest 已对齐为当前 `v0.99.1` loader 产物：
  - 包含 `id`
  - `name`
  - `author`
  - `description`
