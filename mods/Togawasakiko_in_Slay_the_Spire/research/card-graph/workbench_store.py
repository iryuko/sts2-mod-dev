...
class WorkspaceStore:
    ...
    def _assert_storage_paths(self):
        for name in ('workspace.json','.workspace.lock','history','uploads'):
            path=self.root/name
            if path.is_symlink() or not path.resolve().is_relative_to(self.root):
                raise CorruptWorkspace('工作区不允许存储路径为符号链接','unsafe_storage_path')
    ...
    def _commit(self, state):
        ...
        if self.path.exists():
            ...
            if backup.is_symlink(): raise CorruptWorkspace('备份文件不允许为符号链接','unsafe_storage_path')
