...
## 一点五点零 2026-04-20 `Symbol III` 卡面动态格挡值修复

- 已修正 `Symbol III` 仅在结算时增长、但卡面 `{Block}` 数值不随本场战斗打出次数更新的问题
- 当前实现改为：
  - 使用持久 `BlockVar` 作为 `CanonicalVars`
  - 按本场战斗内已打出次数实时刷新当前显示格挡值
  - 未升级时：
    - 第一次显示 `3`
    - 打出后下次显示 `6`
    - 再次打出后显示 `9`
  - 升级后：
    ...
    - 后续每次仍额外 `+3`
- 同时补上了战斗结束重置：
  - `AfterCombatEnd(...)` 会把 `Symbol III` 的战斗内计数清零
  - 下场战斗重新从初始格挡值开始显示
...
- 已新增并接入正常奖励池的第五批补充牌：
  ...
  - `Housewarming / 乔迁`
    ...
    - 升级后获得 `Innate`
    - 当前实现为生成临时修饰版本，不改原始 `Barking Barking Barking` 定义
...
- 当前面板行为：
  ...
  - 选中曲目后停止原版 run music，并用本地 `AudioStreamPlayer` 播放选中的音频文件
  - 下方提供可拖动进度条，直接映射到当前音频文件的真实播放进度
  - 选回 `Off (null)` 后，停止自定义播放并恢复原版 run music / ambience 更新链
...
- 新增：`Two Moons Deep Into The Forest` 的“本战斗减费”状态跨战斗残留
  - 已定位到 `TogawasakikoCombatWatcherPower` 的本战斗歌曲计数没有在新战斗显式清空
  - 已新增 watcher `ResetCombatState()` 并在进入新 combat room 时主动 reset
  - 已同步刷新所有 `Two Moons` 的本战斗费用
  - 已重新构建并安装
  - 待实机复测
...
- `TwoMoonsDeepIntoTheForest`
  - 单体攻击 `16`
  - 本场战斗中每打出 `1` 张不同名称的 `song`，费用减少 `1`
  - 当前由 `TogawasakikoCombatWatcherPower` 按 `Entry` 去重统计，并刷新所有副本费用
  - 当前刷新已覆盖 `player.Deck + combat piles`，并在每次重算前强制回到基础费用 `4`
  - 升级后伤害 `16 -> 21`
- `Sophie`
  - 获得格挡
...
- `Notebook`
  ...
  - `Completeness` 已删去描述中的手写 `NL + Retain / 保留`，升级时只保留真实关键词 `Retain`
...
- 当前仍需优先做实机验证，尤其是：
  ...
  - `TwoMoonsDeepIntoTheForest` 的去重计数与改费同步
