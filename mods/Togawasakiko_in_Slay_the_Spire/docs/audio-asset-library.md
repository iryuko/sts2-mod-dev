...
初建日期：2026-04-04；现状校订：2026-07-26

> 档案状态：目录和命名规则仍有效；runtime 状态以 [音频轨道登记](audio-track-registry.md) 和 [资产状态](asset-status.md) 为准。
...
- 当前已接入 `23` 首 jukebox 曲目、`1` 首事件音乐和 `1` 个角色选择音效
- jukebox 目标行为是选曲后跨所有 room 持续播放，包括 combat；选择 Off 或离开 run 时停止
- Mac/Win 换房生命周期仍需以同一 release 包做实机回归
