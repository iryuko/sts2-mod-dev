...
- 需要复测：
  ...
  - `Two Moons Deep Into The Forest` 在下一场战斗开始时是否恢复为 `4` 费
    - 2026-04-25 后续平衡调整后应改测是否恢复为 `7` 费
...
- 本轮修改：
  ...
  - `RefreshTwoMoonsCosts(...)` 改为覆盖 `player.Deck + combat piles`（2026-04-25 已废弃，见下方追记）
  - `TwoMoonsDeepIntoTheForest.RefreshCostFromCombatState()`（2026-04-25 已废弃，见下方追记）
    - 每次先强制回基础费用 `4`
...
#### 2026-04-25 追记：`Two Moons` 费用残留修法再次更正

- 上述“覆盖 `player.Deck + combat piles` 并重算本场费用”的方向已废弃：
  - 直接触碰 deck 实例并写入 `SetThisCombat(...)` 会把本战斗费用修正留在持久牌对象上
  - 这正是“用一次歌曲牌后整 run 减费，重启后恢复”的来源
- 2026-04-25 二次更正：动态 hook 方向仍不够贴近这张牌的原版参照，已再次废弃：
  - 用户指出应参考 `Banshee's Cry`
  - 已反编译确认 `Banshee's Cry` 是卡本体在 `AfterCardEnteredCombat(...)` / `AfterCardPlayed(...)` 中对自身调用 `EnergyCost.AddThisCombat(...)`
  - `Two Moons` 现改为同一路线：卡本体按本战斗历史和后续打出的 `Song` 张数给自身追加本战斗 local cost modifier
  - 2026-04-25 后续平衡调整：取消“不同名称 / Entry 去重”限制，基础费用改为 `7`，升级伤害改为 `20 -> 30`
  - `TogawasakikoCombatWatcherPower.TryModifyEnergyCostInCombat(...)` 与 `RefreshTwoMoonsCosts(...)` 已移除，避免全局 hook 继续污染后续战斗 / run
  - `ClearPersistedTwoMoonsCostModifiers(...)` 仅用于清掉旧版本留在 deck 实例上的 local modifier
  - 不依赖 `AfterCombatEnd(...)` 清理费用，避免再次接触战斗胜利 / 奖励发放时序
