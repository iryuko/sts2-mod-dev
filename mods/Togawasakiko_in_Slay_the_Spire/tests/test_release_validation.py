...
class ReleaseValidationTests(unittest.TestCase):
    ...
    def test_expected_version_mismatch_fails(self) -> None:
        unexpected_version = "0.2.1" if self.version == "0.2.2" else "0.2.2"
        result = self.run_validator(unexpected_version)
        self.assertEqual(1, result.returncode)
        self.assertIn(f"expected version {unexpected_version}", result.stderr)
